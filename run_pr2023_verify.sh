...
sudo rm -rf "$HOME/tunix-pr1983/artifacts/qwen3_dist_gsm8k/checkpoints"

echo -n "waiting for TPU HBM to be free"
for _ in $(seq 1 30); do
  if docker run --rm --privileged --net=host \
       us-central1-docker.pkg.dev/cloud-tpu-multipod-dev/sizhi/tunix_base_image:pr1983-wandb \
       python3 -c "import jax,sys; sys.exit(0 if jax.local_devices()[0].memory_stats()['bytes_in_use']==0 else 1)" \
       >/dev/null 2>&1; then
    echo " - free"; break
  fi
  echo -n "."; sleep 10
