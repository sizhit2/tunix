...
def create_model(
  ...
  Args:
    ...
    dtype: Parameter dtype the safetensors are loaded as. The rollout serves
      bfloat16; the trainer may load float32 master weights (as
      examples/math_gsm8k/qwen3_grpo_demo.py does for its actor) and stage a
      bfloat16 copy for weight sync via `TrainingConfig.weight_sync_dtype`.
